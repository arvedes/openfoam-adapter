        ...
        for (uint j = 0; j < patchIDs_.size(); j++)
        {
            ...
            if (dim_ == 2)
            {
                ...
                for (int i = 0; i < faceNodesSize; i++)
                {
                    ...
                    {
                        WarningInFunction
                            << "It seems like you are using preCICE in 2D and your geometry is not located int the xy-plane. " << nl
                            << "The OpenFOAM adapter implementation supports preCICE 2D cases only with the z-axis as out-of-plane direction. " << nl
                            << "Please rotate your geometry so that the geometry is located in the xy-plane. " << nl
                            << "If you are running a 2D axisymmetric case just ignore this." << nl;
                        break;
                    }
                }
            }
